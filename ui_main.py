...
class SyncApp(BaseObject):
    ...
    def _init_pyqt_ui(self):
        ...
        # 设置应用程序图标
        # 图标路径只计算一次，供 app 和 window 共享
        self._icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "smart_file_icon.png")
        if os.path.exists(self._icon_path):
            self.app.setWindowIcon(QIcon(self._icon_path))
        ...
        # 同步应用到主窗口（图标路径已在上面计算过）
        if os.path.exists(self._icon_path):
            self.window.setWindowIcon(QIcon(self._icon_path))
        ...
        if QFileSystemModel is not None:
            ...
            # 使用代理模型显示中文列名
            if PYQT_AVAILABLE:
                proxy_model = UIUtils.create_chinese_header_proxy_model()
