...
### 5. 文件夹大小
- **文件夹分析**：扫描指定文件夹的所有子文件夹
- **大小统计**：计算每个子文件夹的总大小和文件数量
- **排序展示**：按文件数量降序排列，快速定位文件最多的文件夹
- **信息丰富**：显示文件夹名称、路径、大小、文件数量和修改时间
- **批量操作**：支持复制、移动、删除选中的文件夹
- **单位换算**：自动将文件夹大小转换为合适的单位（B、KB、MB、GB、TB）
- **智能排序**：点击表头可按任意列排序，大小和数量列按实际数值排序
- **进度监控**：实时显示扫描进度和当前处理的文件夹
...
- **主要依赖**：os, shutil, hashlib, datetime, threading
